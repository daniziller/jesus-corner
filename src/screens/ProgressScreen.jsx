...
export default function ProgressScreen({ session, blocks, sessionsByBlock, onNavigate }) {
  ...
  const sinceLabel = (() => {
    ...
    // Mês em minúsculas, como no quadro ("Desde março · dois placares, não sete").
    const monthName = new Date(y, m - 1, 1).toLocaleDateString(lang === 'en' ? 'en-US' : 'pt-BR', { month: 'long' })
    return L('sinceMonthFull', { month: lang === 'en' ? monthName : monthName.toLowerCase() })
  })()
  ...
  const fmtPct1 = n => Number(n).toLocaleString(locale, { maximumFractionDigits: 1 })
        ...
        <div style={styles.bibleCard}>
          ...
          <div style={styles.blockList}>
            {blocks.map(block => {
              ...
              // Nome curto do bloco (quadro 5b: "Históricos", "Poéticos"…).
              const name = lang === 'en' ? (block.shortNameEn ?? block.nameEn) : (block.shortName ?? block.name)
              return (
          ...
          <p style={styles.bibleFooter}>
            {L('wholeBibleFooter', { pct: fmtPct1(session.biblePercent), done: session.chaptersRead.toLocaleString(locale), total: session.totalChapters.toLocaleString(locale) })}
          </p>
        </div>
...
const styles = {
  ...
  consistencyLabel: {
    ...
    lineHeight: 1, color: 'rgba(255,255,255,.45)', margin: '0 0 14px',
  },
  ...
  consistencySub: { fontFamily: 'var(--font-bento)', fontSize: 13.5, fontWeight: 600, lineHeight: 1, color: 'rgba(255,255,255,.5)' },
  ...
  bibleLabel: {
    fontFamily: 'var(--font-bento)', fontSize: 10.5, fontWeight: 800, letterSpacing: '.12em', textTransform: 'uppercase',
    lineHeight: 1, color: 'var(--bento-t4)', margin: '0 0 14px',
  },
  ...
  bibleBlockLine: { fontFamily: 'var(--font-bento)', fontSize: 12.5, fontWeight: 500, lineHeight: 1.3, color: 'var(--bento-t3)', margin: '0 0 18px' },
  ...
  blockRowName: { flex: 1, minWidth: 0, fontFamily: 'var(--font-bento)', fontSize: 13.5, fontWeight: 700, lineHeight: 1, color: 'var(--bento-ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
