...
// 'all' | 'year' | '30d' → data de corte (string YYYY-MM-DD, comparável
// direto com a coluna `data` de session_seconds/chapters_read) ou `null`
// pra "sem corte" (desde o começo). `from` é o dia de referência (default
// agora), só existe pra dar pra testar com uma data fixa.
...
// linha).
export function mostCommonHour(rows, passo, sinceDate = null) {
  const filtered = rows.filter(r => r.passo === passo && r.created_at && (!sinceDate || r.data >= sinceDate))
...
export function chaptersReadInPeriod(chapterReadRows, sinceDate) {
  if (!sinceDate) return chapterReadRows.length
  const cutoff = new Date(sinceDate)
  return chapterReadRows.filter(r => r.created_at && new Date(r.created_at) >= cutoff).length
}
...
export function reflectionDaysInPeriod(dailyRoutine, sinceDate) {
  return Object.entries(dailyRoutine || {}).filter(
    ([date, day]) => day?.reflection && (!sinceDate || date >= sinceDate)
  ).length
}
