// Sugestão de "pergunta da semana" pro Plano do grupo — quadro 22d do
// redesign Bento. Mesmo espírito de api/generate-reflection-questions.js:
// GET público, cacheado na borda (a pergunta sugerida é igual pra quem abrir
// o mesmo capítulo — o líder sempre revisa/edita antes de publicar, ver
// README "a voz na Comunidade continua humana" — então não há nada
// específico de usuário aqui pra justificar autenticação nem gate de IA por
// pessoa).
import { suggestWeeklyQuestion } from './_lib/ai.js'
...
const MAX_CHAPTER_SPAN = 10 // mesma trava de generate-reflection-questions.js — só contra abuso
...
function fetchBookChapters(folder, bookName) {
  ...
  const promise = fetch(`${APP_URL}/bible-text/${folder}/${slugify(bookName)}.json`)
    ...
    .catch(err => {
      console.error('[suggest-weekly-question] failed to fetch book text:', bookName, err.message)
      return null
    })
...
export default async function handler(req, res) {
  ...
  let question
  try {
    question = await suggestWeeklyQuestion({ book, chStart: startNum, chEnd: endNum, chapterText, bookInfo, lang: cleanLang })
  } catch (err) {
    console.error('[suggest-weekly-question] AI call failed:', err.message)
